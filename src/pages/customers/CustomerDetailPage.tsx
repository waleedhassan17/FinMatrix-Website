...
export default function CustomerDetailPage() {
  ...
  if (isError || !data) {
    return (
      <Card className="p-xl">
        <p className="text-label-lg text-text-primary">Customer not found</p>
      ...
      <Card className="p-lg">
        <div className="flex flex-wrap items-start justify-between gap-md">
          <div className="min-w-0">
            ...
            {customer.company && (
              <p className="text-body-sm text-text-secondary">{customer.company}</p>
            )}
            <p className="mt-xxs text-body-sm text-text-secondary">
...
function StatementTab({ customerId }: { customerId: string }) {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['customers', customerId, 'statement', startDate, endDate],
