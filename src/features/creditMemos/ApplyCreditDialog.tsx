...
import { maxApplicable } from '@/models/creditMemo';
import type { AllocationRow } from '@/models/payment';
import { getOutstandingInvoices } from '@/networks/sales/paymentNetwork';
import { formatMoney } from '@/utils/money';

/**
 * Put a credit against one of the customer's open invoices.
 *
 * The mobile app picks the invoice and then fixes the amount at
 * `min(creditBalance, invoiceBalance)` with no way to change it. The API takes
 * an `amount`, so partial application is supported — here the maximum is
 * pre-filled but editable, which is what lets one credit be split deliberately
 * across two invoices.
 *
 * Open invoices come from the payments module's outstanding endpoint: the same
 * question ("what does this customer still owe?") with the same answer.
 */
  ...
  return (
    <AlertDialog.Root open={open} onOpenChange={onOpenChange}>
      <AlertDialog.Portal>
        ...
        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-50 w-[calc(100vw-2rem)] max-w-[32rem] -translate-x-1/2 -translate-y-1/2 rounded-lg bg-surface p-xl shadow-lg">
          ...
          <AlertDialog.Description className="mt-xs text-body-md text-text-secondary">
            {formatMoney(creditBalance)} available. Pick an invoice, then confirm
            how much of the credit to put against it.
          ...
          <div className="mt-lg max-h-64 overflow-y-auto rounded-md border border-border-light">
            {isLoading ? (
              ...
              <p className="p-md text-body-sm text-text-tertiary">
                This customer has no open invoices to credit.
              </p>
            ) : (
              invoices.map((inv) => (
                <button
                  ...
                  className={cn(
                    ...
                    selected?.documentId === inv.documentId
                      ? 'bg-primary-tint'
                      : 'hover:bg-surface-hover',
                  )}
                >
                  <span className="flex-1 text-body-sm text-text-primary">
                    {inv.documentNumber || inv.documentId.slice(0, 8)}
                    <span className="ml-xs text-caption text-text-tertiary">
                      due {inv.dueDate || '—'}
                    </span>
                  </span>
                  <span className="text-body-sm text-text-primary tabular">
            ...
            <div className="mt-lg">
              <Input
                ...
                hint={
                  tooMuch
                    ? undefined
                    : `Up to ${formatMoney(ceiling)} — whichever runs out first, the credit or the invoice.`
                }
              />
            </div>
          )}
